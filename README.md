...
The README expects these two files:

```text
docs/images/trustfix-dashboard.png
docs/images/trustfix-workflow.png
```

### Option A — Upload with GitHub

1. Open the `trustfix.ai` repository on GitHub.
2. If `docs/images/` does not exist, choose **Add file → Create new file**.
3. In the filename box, enter `docs/images/.gitkeep` and commit it. This creates the folder path.
4. Open `docs/images/`.
5. Choose **Add file → Upload files**.
6. Upload your two screenshots.
7. Name them:
   - `trustfix-dashboard.png`
   - `trustfix-workflow.png`
8. Commit the files to `main`.

GitHub will automatically render the screenshots in this README because the image links use repository-relative paths.

### Option B — Add them locally
...
Copy the two image files into that folder and name them:
...
git commit -m "Add professional README and TrustFix screenshots"
git push
```
...
- Remove or blur real names, email addresses, street addresses, phone numbers, tokens, IDs, financial information, or other customer data.
- Use PNG or WebP for clean UI screenshots.
- Aim for approximately **1200–1800 px wide**.
- Avoid screenshots with large amounts of empty browser chrome.
- Use screenshots that demonstrate different roles or workflows rather than two nearly identical screens.

If your screenshots show different features than the suggested captions, change the `alt` text and caption directly beneath each image.
